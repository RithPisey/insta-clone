...
function Post({ id, userImg, caption, username, postImg }) {
		...
		<div className='bg-white my-7 border-rounded-sm '>
				...
				<div className='flex justify-between pt-4'>
					<div className='flex space-x-4'>
						{hasLiked ? (
							<HeartIconFilled
								onClick={likePost}
								className='btnPost text-red-600'
							/>
						) : (
							<HeartIcon onClick={likePost} className='btnPost' />
						)}

						<ChatBubbleOvalLeftIcon className='btnPost' />
			...
			{session && (
				<form className='flex items-center p-4'>
					<FaceSmileIcon className='h-7 mr-4' />
					<input
						value={comment}
